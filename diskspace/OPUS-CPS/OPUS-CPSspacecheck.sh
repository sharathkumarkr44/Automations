...
if [[ \$value -ge 80 ]]
then
echo -e "$podname$i isfor \$value"
fi
EOF
done
done

cat tmp.log | grep isfor >>space.log
rm tmp.log
if [[ -s space.log ]]
then
while read line
...
echo "Subject: File system space alerts--OPUS & CPS"
echo "Content-Type: text/html"
cat space_check.html
) | /usr/sbin/sendmail -t

cd $Homdir
./OPUS-CPSspaceclear1.sh $(cat space.log | awk '{ print $1 }')

elif [[ ! -s space.log ]]
then
printf  "Hi team,\n\n All servers filespace is well before the threshold. " | mail -s "File system space GREEN--OPUS & CPS" -r "sk742g@att.com" DL-OPUS-AlertMonitor@att.com 
exit
fi
