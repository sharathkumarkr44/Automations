...
. /opt/app/workload/RPSScripts/sk742g/callvumonitoring/config
...
do
	env=`echo $IDS_Portal_url | awk -F "/" '{print $3}' | awk -F "." '{print $1}'`
        /usr/bin/wget -q -S -O- $IDS_Portal_url --no-check-certificate > $LOG_DIR/IDS_Portal_${env}.log 2>&1
...
do
        env=`echo $IDREG_url | awk -F "/" '{print $3}' | awk -F "." '{print $1}'`
        /usr/bin/wget -q -S -O- $IDREG_url --no-check-certificate > $LOG_DIR/IDREG_${env}.log 2>&1
...
do
        env=`echo $AttUtils_url | awk -F "/" '{print $3}' | awk -F "." '{print $1}'`
        /usr/bin/wget -q -S -O- $AttUtils_url --no-check-certificate > $LOG_DIR/AttUtils_${env}.log 2>&1
        ...
        then
                ...
                then
                        printf "<tr>
                                <td style='background-color:#FFFFFF;font-weight:bold'>${env}</td>
                                <td style='background-color:#FFFFFF'>${html}</td>
                                <td style='background-color:#32CD32'>${msg}</td>
                                </tr>">>AttUtils_check.html
                else
                        printf "<tr>
                                <td style='background-color:#FFFFFF;font-weight:bold'>${env}</td>
                                <td style='background-color:#FFFFFF'>${html}</td>
                                <td style='background-color:#F81503'>${msg}</td>
                                </tr>">>AttUtils_check.html
                fi
        else
                msg="Unable to establish connection"
                ...
                printf "<tr>
                        <td style='background-color:#FFFFFF;font-weight:bold'>${env}</td>
                        <td style='background-color:#F81503'>${html}</td><td style='background-color:#F81503'>${msg}</td>
                        </tr>">>AttUtils_check.html
        fi
done<$AttUtils_url
